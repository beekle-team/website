...
- **画像と見出しだけでも要点が伝わる** — トップ・LP・コラムは流し読みを前提にし、課題、Before/After、仕組み、手順、比較を大きな画像で説明する。本文の途中に挿絵を足すだけで終えず、画像を読む順番が説明の順番になるように配置する。
- **詳しい情報はHTMLに保持する** — 根拠、条件、役割分担などの本文は削らず、必要に応じて利用者が開ける詳細領域へ整理する。料金や適用条件など判断に直結する要点は常時表示。画像には内容に即したaltとキャプションを付ける。
