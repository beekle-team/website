...
- `tasks.md` はローカルの作業管理用として使い、Gitに含めない。
...
- コードやドキュメントの変更を依頼された場合は、実装・必要な検証・セルフレビュー・コミット・push・PR作成までを標準の作業範囲とする。ローカル修正だけで完了にせず、各工程で確認を求めずに進める。
- セルフレビューでは、要件との一致、差分の妥当性、既存機能への影響、表示文言とリンク先の整合性、検証結果を確認し、見つけた問題を修正してからPRを作成する。
- 他の作業の未コミット変更を混ぜず、依頼された変更だけをコミット・PRに含める。
- PRには変更の目的・内容・検証結果を記載し、完了報告にはPRのURLと未解決事項があればその内容を添える。権限や外部サービスの問題でPRを作成できない場合は、その理由と残作業を明記する。
- マージ・本番反映は自動では行わず、ユーザーから明示的な依頼があった場合に進める。
