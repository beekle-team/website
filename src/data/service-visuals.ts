...
const prototype: ServiceVisual = {
  ...
  alt: '業務を整理し、試作画面を確認して、開発範囲を決める流れ',
  caption:
...
const knowledge: ServiceVisual = {
  ...
  alt: '仕様書や対応履歴を検索し、回答と参照した資料を並べて確認する流れ',
  caption:
...
const documents: ServiceVisual = {
  ...
  alt: '帳票の項目を読み取り、担当者が原本と照合して業務システムへ登録する流れ',
  caption:
...
const approval: ServiceVisual = {
  ...
  alt: '複数システムの情報から作業案を準備し、人の承認後に実行して履歴を残す流れ',
  caption:
...
export const serviceVisuals: Record<string, ServiceVisual> = {
  ...
  'sales-data-coaching': {
    ...
    alt: '部門ごとの記録を集め、同じ基準で整理し、担当者が改善策を決める流れ',
    caption:
