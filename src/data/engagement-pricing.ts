...
// 2026-09-18 ユーザー判断: Beekleは基本ゼロスタートのみ。0円の叩き台は「営業として成立するかを双方が判断できる状態」まで作る。
...
export const ZERO_START_SCOPE_RULE =
  '0円の叩き台は、NDA締結後にお預かりした実データでコアのロジックが動き、営業として成立するかを双方が判断できる状態までです。対象は1業務・1データ源。全量データの投入、権限、既存システム連携、確認画面、評価セットの整備、運用は、準委任（月額）のPoC以降で行います。';
