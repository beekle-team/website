...
export const industryPageConfig: Record<string, IndustryPageConfig> = {
  ...
  'manufacturing-parts-knowledge-search': {
    ...
    headline: '「この部品は使えるか」「後継品はどれか」を、根拠つきで確認',
    heroLead:
    ...
    rtb: {
      eyebrow: 'WHY BEEKLE',
      title: '適合情報の照合と、検索システムの運用を経験しています',
      lead: '適合情報を扱ったPoCと、カスタマーサポートでの検索の本番運用。それぞれの経験を、御社の資料と業務に合わせて活かします。',
      items: [
        {
          title: 'カタログPDFの適合情報を、既存マスタと照合した',
          description:
            '商品カタログPDFの表を読み取り、表記ゆれを整理して既存マスタと照合し、CMS取込用CSVを出力するPoCを作りました。曖昧な候補だけを人が確認する画面も試作しています。入力資料から照合・確認・出力までを一続きで実装した経験です。',
        },
        KBF_REQUIREMENTS,
        {
          title: '実際に本番で運用経験あり',
        ...
        KBF_PMR,
        {
          ...
          description:
            '要件整理から提案依頼書（RFP）の作成を代行し、そのまま要件定義・デザイン・業務整理・開発まで担当しました。資料と担当者へのヒアリングで業務を整理し、作る範囲を決めて実装まで進めています。',
        },
        {
          title: '基本ロジックが動くものを、相談段階で確かめる（ゼロスタート）',
          description:
            'ヒアリングをもとに、基本ロジックまで組み込んだ動くデモを作ります。実際に触りながら、できること、追加で必要な機能、費用を一緒に確認できます。初回相談と、完成イメージを合わせる簡易デモは無料です。実データでの精度検証や個別の連携を含む開発は、範囲と費用をご提示してから進めます。',
        },
        KBF_SPEED,
      ],
      note: PMR_NOTE,
