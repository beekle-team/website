...
export const industryPageConfig: Record<string, IndustryPageConfig> = {
  ...
  'manufacturing-parts-knowledge-search': {
    ...
    headline: '「この部品は、この製品に使えるか」に、根拠つきですぐ答えられるように',
    heroLead:
      '型番・仕様・適合・などの複雑な関係もAIで自動で探し回答し、元の仕様書と該当箇所まで示す社内ナレッジ検索を作ります。文書検索で十分な質問にはそのまま答え、関係をたどらないと答えられない質問では、製品・部品・型番・仕様の関係を設計通り回答します。',
    primaryOutcome:
      'まず問い合わせが集中している1製品群。実際に来ている質問で検索精度を確認し、必要な技術を選定します。',
    ...
    rtb: {
      ...
      items: [
        ...
        KBF_PMR,
        {
          ...
          description:
            '要件整理から提案依頼書（RFP）の作成を代行し、そのまま要件定義・デザイン・業務整理・開発まで担当しました。適合情報そのものを扱った案件もあります。商品カタログPDFに残っていた適合情報を、表の読み取りと表記ゆれの整理を通して既存マスタと照合し、CMS取込用のCSVを出すところまで一続きで作りました。曖昧な候補だけ人が確認する画面も試作しています。関係をたどる検索は、カスタマーサポート向けに実データを投入して運用しているHybrid GraphRAGが本番の実績で、機種・部品・作業の関係をたどる用途では航空機の整備マニュアルの改定確認という課題にも取り組んでいます（守秘義務があるため詳細は書けません）。',
        },
        KBF_ZERO,
        KBF_SPEED,
      ],
