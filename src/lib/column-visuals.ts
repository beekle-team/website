...
function buildContactCta(source: string, intent: string): string {
  return buildCtaCard({
    ...
    body: 'Beekleでは、AI活用や業務システム開発について、作るものの整理、費用感、進め方まで一緒に確認します。まだ曖昧な段階でも、最初に何を決めればよいかから相談できます。',
    label: 'お問い合わせはこちら',
  });
}
...
function buildZeroStartConsultCta(source: string, intent: string): string {
  return buildCtaCard({
    ...
    body: 'Beekleのゼロスタートなら、最短1〜2週間を目安に（規模により変動）動く試作品を作り、「本当に業務で使えるか」を実際に触って確認してから本格開発に進めます。',
    label: 'ゼロスタートについて相談する',
  });
}

// AI検索が買い手を送り込む記事（見積・依頼書・顧客データ・発注準備）向けの、意図別リード獲得CTA。
...
const CONSULT_CTAS: Record<string, ConsultCta> = {
  GENAI_START_CONSULT: {
    intent: 'genai-adoption',
    title: '自社でAIを使うなら、どの業務から始めるか相談する',
    body: '資料探し、問い合わせ対応、書類の転記、確認作業など、どこから始めると効果を見込みやすいかを一緒に整理します。',
    label: 'AI導入の進め方を相談する',
  },
  GENAI_ROI_CONSULT: {
    intent: 'genai-roi',
    title: 'AI導入が費用に見合うか相談する',
    body: '導入費だけでなく、運用費、削減できる作業時間、社内承認に必要な説明まで、投資判断に使える形で整理します。',
    label: '費用対効果を相談する',
  },
  DOC_SEARCH_CONSULT: {
    intent: 'document-ai-search',
    title: '社内資料をAIで探せるか相談する',
    body: 'PDF、規程、マニュアル、過去対応が散らばっている場合に、質問すると根拠つきで答えられる仕組みにできるか確認します。',
    label: '社内資料AIを相談する',
  },
  AI_ACCURACY_CONSULT: {
    intent: 'ai-accuracy',
    title: 'AIの回答ミスを減らせるか相談する',
    body: 'AIがそれっぽく間違える、根拠が見えない、社内で使うには不安。業務で使う前に確認すべき点を整理します。',
    label: '回答精度を相談する',
  },
  DX_WORKFLOW_CONSULT: {
    intent: 'dx-workflow',
    title: '紙・Excel・属人作業をどこから見直すか相談する',
    body: '現場の手作業、二重入力、担当者しか分からない業務を洗い出し、AIやシステムで軽くできる範囲を整理します。',
    label: '業務改善を相談する',
  },
  ESTIMATE_CONSULT: {
    intent: 'estimate',
    title: 'この見積もりで進めて大丈夫か確認する',
  ...
  TECH_REVIEW_CONSULT: {
    intent: 'tech-review',
    title: '社内データや既存システムの不安を相談する',
    body: '社外にデータを出せない、既存システムとつながるか分からない、導入後に運用できるか心配といった点を確認します。',
    label: '技術面を相談する',
  },
...
const SERVICE_BRIDGES: Record<string, ServiceBridge> = {
  ...
  RAG_SERVICE_BRIDGE: {
    ...
    title: 'Beekleの社内資料AI検索',
    body: '社内資料から根拠付きで答えるAIを構築します。回答が間違いやすい、根拠が見えない、資料更新に追いつかないといった問題まで設計します。',
    label: 'サービス内容を見る',
  },
