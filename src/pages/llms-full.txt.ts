...
2. ゼロスタート（0円の検証用デモ）: 対象業務を絞り、実データで動くデモを作る。業務適合・要件・データ・利用者の反応を確認し、双方で 開発する / 見送る / 範囲を変える を判断する。合わなければここで終了し、費用は発生しない。Beekle側も、データや実現性から難しい案件は断る。見送りの場合も理由を残す
3. PoC（準委任・月額）: 望みがある場合だけ進む。デモで分かったデータの状態と難しさをもとに、確かめる範囲と体制を再見積もりする。単価は職種別に公開（/strengths#rates、税別）
4. MVP・本開発・運用（準委任・月額）: 同じ体制で、要件定義・設計・デザイン・実装・インフラ・運用まで続ける

- 契約形態: 準委任。月単位でチームが参画する。PoC以降の費用は「単価 × 期間 × 体制」で決まり、金額を1つの固定レンジで示さない
