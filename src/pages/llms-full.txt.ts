...
Beekleは基本的にゼロスタートのみで受ける。固定額の段階別料金表は置かない。

1. 無料相談・NDA締結: 要件が固まっていない段階でも可。複数社比較中でも可。NDAを結んだうえで実際の業務データを預かり、状態を確認する
2. ゼロスタート（0円の検証用の叩き台）: 対象業務を絞り、実データで動く叩き台を作る。0円の範囲は、必要な画面と業務ロジックが動き、開発を進める価値を双方で判断できる状態まで。AI DXはまず1業務に絞り、入力・保存・条件分岐・結果の確認など、業務を試すための処理も実装します。データと実装範囲は事前に合意し、本番向けの品質・精度の検証、連携や機能の拡張、運用の整備は別途見積もります。業務適合・要件・データ・利用者の反応を確認し、双方で 開発する / 見送る / 範囲を変える を判断する。合わなければここで終了し、費用は発生しない。Beekle側も、データや実現性から難しい案件は断る。見送りの場合も理由を残す
3. PoC（準委任・月額）: 望みがある場合だけ進む。叩き台で分かったデータの状態と難しさをもとに、確かめる範囲と体制を再見積もりする。単価は職種別に公開（/strengths#rates、税別）。モデルケースの最小構成はAIエンジニア1名＋エンジニア1名 × 1か月〜、約208万円〜（税別、単価の下限で計算。AIエージェントのように連携先が多い場合は1.5か月〜、約312万円〜）
4. MVP・本開発・運用（準委任・月額）: 同じ体制で、要件定義・設計・デザイン・実装・インフラ・運用まで続ける。PoCから本番展開までの全体は、多くの案件でAIエンジニア1名＋エンジニア1名 × 3〜4か月なら、約624万〜832万円（税別）。判断の軸として置いている数字で、対象範囲と連携先が増えれば期間が延び、金額も同じ掛け算で増える

- 契約形態: 準委任。月単位でチームが参画する。PoC以降の費用は「単価 × 期間 × 体制」で決まり、金額を1つの固定レンジで示さない。サービスごとのモデルケースに、体制と最短期間から計算した目安を1行ずつ置いている
- 要件定義のみ、RFP作成のみの依頼にも対応する（整理した要件で他社へ発注する前提でも可）
- 経営導入プラン壁打ち: 35,000円（税別、税込38,500円）/ オンライン90分・単発。経営DXページから申込。経営課題、AI・DX導入候補、優先順位、次のアクションを導入検討メモにまとめる。詳細な要件定義・データ調査・開発は含まない。継続契約は必須ではなく、継続の有無にかかわらず壁打ち料金は発生する。
- 公開価格（AI導入・経営DX支援 / 税別）: AI顧問 200,000円 月〜（最低3ヶ月）/ AI導入・経営DX伴走 800,000円 月〜（最低3ヶ月）/ AI・DX推進室 1,200,000円 月〜（最低3ヶ月）/ AI・経営DX診断 無料（NDA後に実データで検証用デモを作成。進めない場合の費用なし）
